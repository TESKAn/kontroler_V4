...
#if (SCI_1_HIGHLEVEL_MODE == SCI_HIGHLEVEL_MODE_BUFFERED)
   
    #if (SCI_1_RX_BUFFER_SIZE == 0)
    /* SCI_1 RX buffer can't be set to 0 in buffered mode */
    #error SCI 1 RX buffer is set to 0.
    #endif

    #if (SCI_1_TX_BUFFER_SIZE == 0)
    /* SCI_1 TX buffer can't be set to 0 in buffered mode */
    #error SCI 1 TX buffer is set to 0.
    #endif

#endif

#if (SCI_0_HIGHLEVEL_MODE == SCI_HIGHLEVEL_MODE_BUFFERED)
   
    #if (SCI_0_RX_BUFFER_SIZE == 0)
    /* SCI_0 RX buffer can't be set to 0 in buffered mode */
    #error SCI 0 RX buffer is set to 0.
    #endif

    #if (SCI_0_TX_BUFFER_SIZE == 0)
    /* SCI_0 TX buffer can't be set to 0 in buffered mode */
    #error SCI 0 TX buffer is set to 0.
    #endif

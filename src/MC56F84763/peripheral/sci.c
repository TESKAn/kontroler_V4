...
#pragma unused(pSciBase)
	...
	if (pSciBase == SCI_0)
	{
		...
		#ifdef SCI_0_SCICR3_INIT
		periphMemWrite(SCI_0_SCICR3_INIT, &pSciBase->ctrl3);
		#endif
	}
	else
#endif	
	...
	if (pSciBase == SCI_1)
	{
		...
		#ifdef SCI_1_SCICR3_INIT
		periphMemWrite(SCI_1_SCICR3_INIT, &pSciBase->ctrl3);
		#endif
	}
	else
#endif

#if SCI_2_INIT_USED && defined(SCI_2)
	if (pSciBase == SCI_2)
	{
		#ifdef SCI_2_SCIBR_INIT
		periphMemWrite(SCI_2_SCIBR_INIT, &pSciBase->scibr);
		#endif
		
		#ifdef SCI_2_SCICR_INIT
		periphMemWrite(SCI_2_SCICR_INIT, &pSciBase->scicr);
		#endif

		/* SCI_VERSION_2-only  (see module strucutre and target MCU in arch.h) */
		#ifdef SCI_2_SCICR2_INIT
		periphMemWrite(SCI_2_SCICR2_INIT, &pSciBase->scicr2);
		#endif
		
		#ifdef SCI_2_SCICR3_INIT
		periphMemWrite(SCI_2_SCICR3_INIT, &pSciBase->ctrl3);
		#endif
	}
	else
#endif
